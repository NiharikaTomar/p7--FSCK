  ...
  int limit = size_block - sblock->nblocks;
	...
	while (a < sblock->ninodes) {
      ...
      while (indirect < NINDIRECT) {
				ind = (uint *)((indirect * sizeof(uint)) + (di->addrs[NDIRECT] * BSIZE) + img);
...
int in2 = 0 ;
  ...
  di = (struct dinode*) (img + (BSIZE * sblock->inodestart));
	...
	while (cnt < sblock->ninodes) {
		...
		while (ok1 < NDIRECT) {
          ...
          inc++;
			...
			if (((di->addrs[NDIRECT] * BSIZE) + img) != 0) {
				int ok = 0;
				while (ok < NINDIRECT) {
					ind = (uint *) ((ok * sizeof(uint)) + (di->addrs[NDIRECT] * BSIZE) + img);
					if(*ind != 0){
						inc++;
					}
					ok++;
      ...
      assert(!((di->size > inc * BSIZE || di->size < ((inc - 1) * BSIZE)) && (inc != 0)),
